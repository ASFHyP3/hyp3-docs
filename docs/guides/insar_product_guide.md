...
!!! tip "New: Adaptive Phase Filter parameter is now customizable!"
    ...
    There is now an option to adjust the adaptive phase filter parameter value when submitting On Demand InSAR jobs. This option is currently only available in the API, but is coming soon to Vertex! Refer to the [Adaptive Phase Filter section](#adaptive-phase-filter) for more information.
