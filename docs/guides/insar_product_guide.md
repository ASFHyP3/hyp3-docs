...
!!! important "New Processing Options Released, Water Masking Coming Soon!" 
    ...
    Several new InSAR processing options have been released. In addition to the original options to set the resolution (number of looks), and whether to include the look vector maps and the line-of-sight displacement map, users can now include GeoTIFF outputs for the **wrapped phase**, the **local incidence angle**, and a copy of the **DEM** used for processing. The water masking option listed below is in the process of being implemented and will be available soon. 
