...
The [OPERA Science Data System (SDS)](https://software.nasa.gov/software/NPO-52101-1 "OPERA SDS" ){target=_blank} 
platform is the operational system that generates new OPERA RTC-S1 products as Sentinel-1 acquisitions become 
available as [SLC](https://documentation.dataspace.copernicus.eu/Data/SentinelMissions/Sentinel1.html#sentinel-1-level-1-single-look-complex-slc ){target=_blank}s. 
The OPERA SDS has produced RTC-S1 products for acquisitions over the project area since January 1, 2022.

ASF worked with the OPERA team to implement On Demand processing for
OPERA RTC-S1 products using ASF's [HyP3](https://hyp3-docs.asf.alaska.edu/ "hyp3-docs.asf.alaska.edu") 
platform. The HyP3 On Demand workflow uses the same algorithm that is used for products generated by the SDS, 
and HyP3-generated products were validated to ensure equivalency with SDS-generated products. 
The HyP3 platform was then used to generate OPERA RTC-S1 products back to April 14, 2016.

All OPERA RTC-S1 products, regardless of the production system used for generation, are archived in the 
...
OPERA RTC-S1 products can be accessed using a number of methods: 
...
The naming convention for OPERA RTC-S1 products generated using HyP3 is the same as for 
the OPERA RTC-S1 products generated using the OPERA SDS.

### L2 Radiometric Terrain Corrected (RTC) Files

If you search for OPERA-S1 products in Vertex, the results for each Sentinel-1 burst provide access to a number of 
files. If you click on an item in the left panel of the search results, the associated files are listed in the 
right panel. 
...
## Sentinel-1 RTC Products Available from ASF
...
OPERA RTC-S1 products are output in gamma-0 power with 30-m pixel spacing. The only method currently available for 
ordering products with a different radiometry, scale, or pixel spacing is to order ASF's 
[On-Demand Sentinel-1 RTC products](rtc_product_guide.md) 
